...
contract('HappyCase', accounts => {
  ...
  specify('Steps', async () => {
    ...
    let multisig = await support.gasDiff('Multisig.new', web3, sender, async () => {
      return await Multisig.new(sender, receiver)
    })

    instanceForDigest = await Bidirectional.new(multisig.address, settlementPeriod)
    counterFactory = new InstantiationFactory(web3, multisig)

    // Step 2: Construct Lineup
    lineup = bytecodeManager.constructBytecode(Lineup, 0x0, settlementPeriod, multisig.address)
