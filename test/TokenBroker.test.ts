...
contract('TokenBroker', async accounts => {
  ...
  it('create channel', async () => {
    ...
    await broker.createChannel(token.address, receiver, 100, 1, startChannelValue, { from: sender, gas: 400000 })
  ...
  it('makes deposit', async () => {
    ...
    await token.approve(broker.address, startChannelValue, { from: sender })
    const res = await broker.createChannel(token.address, receiver, 100, 1, startChannelValue, { from: sender, gas: 420000 })
  ...
  it('claimed by receiver', async () => {
    ...
    await token.approve(broker.address, startChannelValue, { from: sender })
    const res = await broker.createChannel(token.address, receiver, 100, 1, startChannelValue, { from: sender, gas: 400000 })
    const channelId = res.logs[0].args.channelId
    ...
    await broker.claim(channelId, startChannelValue, Number(v), r, s, {from: receiver, gas: 200000})
