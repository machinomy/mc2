...
contract BrokerToken is Destructible {
   enum ChannelState { Open, Settling, Settled }
    ...
    function createChannel(address con, address receiver, uint duration, uint settlementPeriod, uint256 value) public returns(bytes32) {
        var channelId = sha3(id++);
        var sender = msg.sender;
        var c = StandardToken(con);
    ...
    function deposit(address con, bytes32 channelId, uint value) public {
    ...
    function claim(address con, bytes32 channelId, uint256 payment, bytes32 h, uint8 v, bytes32 r, bytes32 s) public {
        if (!canClaim(channelId, h, v, r, s)) {
          return;
        }

        settle(con, channelId, payment);
    }
    ...
    function finishSettle(address con, bytes32 channelId) public {
      require(canFinishSettle(msg.sender, channelId));
      settle(con, channelId, channels[channelId].payment);
    }
    ...
    function settle(address con, bytes32 channelId, uint256 payment) {
      var channel = channels[channelId];
